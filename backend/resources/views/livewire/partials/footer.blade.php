    ...
    <div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 lg:pt-20 mx-auto">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
            <div class="flex flex-col col-span-full lg:col-span-1 text-center lg:text-left">
                <!-- Brand name -->
                <a class="flex-none text-2xl rtl:text-right font-bold text-white" href="/" aria-label="Brand">
                    FR3ON GYM
                </a>
            ...
            <div class="col-span-1">
                <h4 class="font-semibold text-gray-100">{{ __('footer.products') }}</h4>

                <div class="mt-3 grid space-y-3">
                    <p>
                        <a class="inline-flex gap-x-2 text-gray-400 hover:text-gray-200"
                            href="/categories">{{ __('footer.categories') }}</a>
                    </p>
                    <p>
                        <a class="inline-flex gap-x-2 text-gray-400 hover:text-gray-200"
                            href="/products">{{ __('footer.all_products') }}</a>
                    </p>
                    <p>
                        <a class="inline-flex gap-x-2 text-gray-400 hover:text-gray-200"
                            href="/products?on_sale=true">{{ __('footer.on_sale_products') }}</a>
                    </p>
                </div>
            </div>
            <!-- End Col -->
            ...
            <div class="col-span-1">
                <h4 class="font-semibold text-gray-100">{{ __('footer.company') }}</h4>

                <div class="mt-3 grid space-y-3">
                    <p>
                        <a class="inline-flex gap-x-2 text-gray-400 hover:text-gray-200"
                            href="/about">{{ __('footer.about_us') }}</a>
                    </p>
                    <p>
                        <a class="inline-flex gap-x-2 text-gray-400 hover:text-gray-200"
                            href="/contact">{{ __('footer.contact_us') }}</a>
                    </p>
                </div>
            </div>
            <!-- End Col -->
            ...
            <div class="col-span-2">
                <h4 class="font-semibold text-gray-100">{{ __('footer.contact_us') }}</h4>

                <form wire:submit.prevent="sendMessage">
                    <div class="mt-4 flex flex-col gap-3 border rounded-lg p-4 bg-gray-800">
                        <input type="text" wire:model.defer="name" placeholder="{{ __('footer.your_name') }}"
                            required
                            class="py-3 px-4 block w-full rounded-lg border text-sm bg-slate-900 text-gray-400" />

                        <input type="email" wire:model.defer="email" placeholder="{{ __('footer.your_email') }}"
                            required
                            class="py-3 px-4 block w-full rounded-lg border text-sm bg-slate-900 text-gray-400" />

                        <textarea wire:model.defer="message" rows="3" placeholder="{{ __('footer.your_message') }}" required
                            class="py-3 px-4 block w-full rounded-lg border text-sm bg-slate-900 text-gray-400"></textarea>
                        ...
                        <button type="submit"
                            ...
                            <span wire:loading.remove>{{ __('footer.send_message') }}</span>
                            <span wire:loading>{{ __('footer.sending') }}</span>
                        </button>
        ...
        <div class="mt-5 sm:mt-12 grid gap-y-2 sm:gap-y-0 sm:flex sm:justify-between sm:items-center">
            <div class="flex justify-between items-center">
                <p class="text-sm text-gray-400">© 2025 FR3ON GYM. {{ __('footer.all_rights') }}</p>
            </div>
            <!-- End Col -->
            ...
            <div class="flex gap-2">
                <!-- Facebook -->
                <a class="w-10 h-10 inline-flex justify-center items-center rounded-lg text-white hover:bg-white/10"
                ...
                <!-- Instagram -->
                <a class="w-10 h-10 inline-flex justify-center items-center rounded-lg text-white hover:bg-white/10"
                    href="https://www.instagram.com/fr3on_gym?igsh=cnR2d2QyMDMxa3cx" target="_blank" rel="noopener">
                ...
                <!-- WhatsApp -->
                <a class="w-10 h-10 inline-flex justify-center items-center rounded-lg text-white hover:bg-white/10"
                    href="https://wa.me/201018993993" target="_blank" rel="noopener">
